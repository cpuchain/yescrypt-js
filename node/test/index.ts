...
import { scrypt_kdf, yescrypt_kdf } from '../index.js';
    ...
    it('Test Cases', () => {
        for (const { passwd, salt, N, r, p, t, useScrypt, output } of cases) {
            const hashed = useScrypt
                ? scrypt_kdf(Buffer.from(passwd), Buffer.from(salt), N, r, p, t)
                : yescrypt_kdf(Buffer.from(passwd), Buffer.from(salt), N, r, p, t);
